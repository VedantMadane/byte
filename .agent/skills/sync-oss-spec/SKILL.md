---
name: sync-oss-spec
description: "Use when byte may have drifted from OSS_SPEC.md. Fetches the latest spec from GitHub, walks its mandates, and fixes each violation so the repository keeps conforming. Runs standalone — it does not shell out to any external validator binary."
---

# Syncing byte with OSS_SPEC.md
