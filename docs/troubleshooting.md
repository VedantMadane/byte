...
| `tray error: ...` (any other message) | The tray icon, its menu, or the underlying windowing event loop failed to initialize or run — for example the OS refused to create a tray icon or the event loop could not start. Rare, and most likely on a headless session, an unusual remote desktop, or a minimal window manager without a notification area. | Confirm a desktop session with a system tray is available, then retry. The CLI (`byte list`, `byte switch`, ...) needs no tray and keeps working regardless. |
